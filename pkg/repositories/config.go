...
func (repo *ConfigRepository) GetConfigByCode(ctx context.Context, code string) (result dtos.ConfigDetail, errorData common.ErrorData) {
	errorData.Err = repo.DB.WithContext(ctx).Table(daos.ConfigTableName).Where("code = ?", code).Find(&result).Error
...
func (repo *ConfigRepository) ListConfig(ctx context.Context, current, pageSize int, order, query string, queryArgs []interface{}) (results dtos.ConfigDetailList, errorData common.ErrorData) {
	db := repo.DB.WithContext(ctx).Table(daos.ConfigTableName)
...
func (repo *ConfigRepository) AddConfig(ctx context.Context, model dtos.ConfigCreate) (result dtos.ConfigDetail, errorData common.ErrorData) {
	errorData.Err = repo.DB.WithContext(ctx).Table(daos.ConfigTableName).Create(&model).Scan(&result).Error
	if errorData.IsNotNil() {
...
func (repo *ConfigRepository) DeleteConfigByCode(ctx context.Context, code string) (errorData common.ErrorData) {
	errorData.Err = repo.DB.WithContext(ctx).Table(daos.ConfigTableName).Where("code = ?", code).Delete(nil).Error
	if errorData.IsNotNil() {
		errorData = daos.ParserDatabaseError(ctx, errorData)
		config.Logger.Error(errorData.Err)
		errorData.ResponseCode = http.StatusInternalServerError
...
func (repo *ConfigRepository) UpdateConfig(ctx context.Context, model dtos.ConfigUpdate) (result dtos.ConfigDetail, errorData common.ErrorData) {
	errorData.Err = repo.DB.WithContext(ctx).Table(daos.ConfigTableName).Save(&model).Find(&result).Error
	if errorData.IsNotNil() {
		config.Logger.Errorf("err: %s", errorData.Err.Error())
		errorData = daos.ParserDatabaseError(ctx, errorData)
		errorData.ResponseCode = http.StatusInternalServerError
